...
var globalNewAssessmentCoolOff int64 = 500
...
func invokeApi(command string, logger *log.Logger) (*http.Response, []byte, error) {
	...
	for {
		...
		if (resp.StatusCode == 503) || (resp.StatusCode == 529) || (resp.StatusCode == 429) {
			// In case of the overloaded server, randomize the sleep time so
			// that some clients reconnect earlier and some later.

			sleepTime := 20 + rand.Int31n(60)

			if logLevel >= LOG_NOTICE {
				logger.Printf("[NOTICE] Sleeping for %v seconds after a %v response", sleepTime, resp.StatusCode)
			}
...
func (manager *Manager) labsRun() {
	...
	if labsInfo.NewAssessmentCoolOff >= 700 {
	...
	for {
		...
		case e := <-manager.internalEventChannel:
			...
			if e.eventType == 429 {
				...
				manager.logger.Printf("[INFO] 429 - active:%v | current:%v | max:%v", activeLabsAssessments, currentLabsAssessments, maxLabsAssessments)
			...
			if e.eventType == INTERNAL_ASSESSMENT_STARTING {
				...
				if logLevel >= LOG_INFO {
					...
					manager.logger.Printf("[INFO] status: active:%v | current:%v | max:%v", activeLabsAssessments, currentLabsAssessments, maxLabsAssessments)
